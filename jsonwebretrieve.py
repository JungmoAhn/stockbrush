...
class WebJsonRetrieval(object):
    ...
    """
    def __init__(self, saved_json_file):
        """ 

        """
        ## parameters
        self.saved_json_file = saved_json_file
...
#        url = URL(self.com_data_full_url)
        ...
        try:
            url_data = urlopen(self.com_data_full_url).readlines()
#            url_data = json.loads(response)
#            url_data = url.download(timeout = 50)
        except:
            url_data = ''
#        url_data = bytes(url_data, 'utf-8')

        f.writelines(url_data) 
        f.close()
